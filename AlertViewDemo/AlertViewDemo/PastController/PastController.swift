...
//        alert.messagePadding = 30
        // messagePadding和messageHeight 不推荐同时使用 会有约束警告 会在下个版本修复 (同时使用也不会影响展示效果)
...
//        dev.imagePlacement = .right
    ...
    private func alertCustomType4() {
        let alert = AEAlertView(style: .defaulted, title: "more action", message: nil)
        alert.actionArrangementMode = .vertical
        let action1 = AEAlertAction(title: "11111") { action in
            alert.dismiss()
        }
        let action2 = AEAlertAction(title: "2222") { action in
            
        }
        let action3 = AEAlertAction(title: "33333\r3333") { action in
            
        }
        action3.numberOfLines = 2
...
class PastController: UIViewController {
    ...
    // 自定义
    let alertCustomTypes = ["动画成功", "动画失败", "继承alertView", "多个按钮"]
