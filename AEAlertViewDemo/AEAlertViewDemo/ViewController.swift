...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let normal = UIButton(frame: CGRect(x: 10, y: 100, width: 100, height: 40))
        normal.backgroundColor = UIColor.red
        normal.setTitle("普通Alert", for: .normal)
        normal.addTarget(self, action: #selector(normalBtn), for: .touchUpInside)
        
        view.addSubview(normal)
        
        let uiNormal = UIButton(frame: CGRect(x: 130, y: 100, width: 100, height: 40))
        uiNormal.backgroundColor = UIColor.red
        uiNormal.setTitle("UIAlert", for: .normal)
        uiNormal.addTarget(self, action: #selector(uiNormalBtn), for: .touchUpInside)
        
        view.addSubview(uiNormal)
...
//        }
    ...
    @objc private func normalBtn() {
        AEUIAlertView.showAlertTextField(nil, "测试text", "输入框", ["cancel"]) { (text) in
            print("\(text)")
        }
    }
    
    @objc private func uiNormalBtn() {
        let view = AEAlertView(alertViewStyle: .TextField, title: "Title", message: nil)
