...
//        let custom = UIView(frame: CGRect(x: 0, y: 0, width: 300, height: 40))
//        custom.backgroundColor = .red
////
//        let alert = AEAlertView(style: .custom, title: "提示", message: "")
//        alert.set(content: text, width: 300, height: 40)
////        alert.set(custom: custom, width: 300, height: 40)
////        alert.textFieldBottomMargin = 100
//
//        let cancel = AEAlertAction.init(title: "取消", style: .cancel) { (action) in
//            print(action.tag)
//            alert.dismiss()
//        }
//        alert.addAction(action: cancel)
//        alert.show()
//        let a = AlertView.init(style: .custom, title: "texxx", message: "1231312")
//        a.titleTopMargin = 50
//        let cancel = AEAlertAction.init(title: "取消", style: .cancel) { (action) in
//            print(action.tag)
//            a.dismiss()
//        }
//        a.addAction(action: cancel)
//        let sub = AEAlertAction.init(title: "dev", style: .defaulted) { (action) in
//            print(action.tag)
//
//        }
//        a.addAction(action: sub)
//
//        a.show()
//        AlertView.show(title: "title", actions: ["cancel", "def"], message: "") { (action) in
//
//        }
        
//        AEAlertView.show(title: "title", actions: ["cancel", "def"], message: "臣本布衣，躬耕于南阳，苟全性命于乱世，不求闻达于诸侯。先帝不以臣卑鄙，猥自枉屈，三顾臣于草庐之中，咨臣以当世之事，由是感激，遂许先帝以驱驰。后值倾覆，受任于败军之际，奉命于危难之间，尔来二十有一年矣。         \r\r先帝知臣谨慎，故临崩寄臣以大事也。受命以来，夙夜忧叹，恐托付不效，以伤先帝之明；故五月渡泸，深入不毛。今南方已定，兵甲已足，当奖率三军，北定中原，庶竭驽钝，攘除奸凶，兴复汉室，还于旧都。此臣所以报先帝而忠陛下之职分也。至于斟酌损益，进尽忠言，则攸之、祎、允之任也。 \r\r愿陛下托臣以讨贼兴复之效，不效，则治臣之罪，以告先帝之灵。若无兴德之言，则责攸之、祎、允等之慢，以彰其咎；陛下亦宜自谋，以咨诹善道，察纳雅言，深追先帝遗诏。臣不胜受恩感激。 \r\r今当远离，临表涕零，不知所言。", bgImage: UIImage(named: "001"), bgImageBottomMargin: 40, titleColor: UIColor.red, defaultActionColor: UIColor.red) { (action) in
//            print(action.tag)
//        }
//        AEAlertView.show(title: "最快的", actions: ["cancel", "def"], message: "确实快") { (action) in
//            print(action.tag)
//        }
        
//        let alert = AETextFieldAlertView.show(title: "此操作需要密码", message: "", bgImage: UIImage(named: "002"), placeholder: "23123", text: "text")
//        alert.textField.backgroundColor = UIColor.clear
//        alert.textField.layer.borderWidth = 1
//        alert.set(custom: custom, width: 300, height: 40)
//
//
...
//        let a = AEAlertView.init(style: .defaulted, title: "11", message: "111")
//        a.show()
//    }
//}
//
//// MARK: - 如果AEAlertView 和 AEAlertView  都无法满足需求 可以使用AEBaseAlertView
...
//
//        cancelButtonColor = UIColor.black
